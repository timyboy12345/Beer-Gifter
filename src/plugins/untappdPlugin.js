...
export default {
    install: (app, options) => {
        ...
        const getUntappdBeer = (bid) => {
            return new Promise((resolve, reject) => {
                app.axios.get(`https://api.untappd.com/v4/beer/info/${bid}?access_token=${credentialsStore.token}`)
                    .then((response) => {
                        resolve(response.data.response.beer);
                    })
                    .catch((err) => {
                        reject(err.response.data);
                    })
        ...
        app.provide('getUntappdBeer', getUntappdBeer);
    }
}
